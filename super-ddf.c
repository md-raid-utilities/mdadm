				  ...
				  unsigned long long *out)
{
	...
	unsigned long long search_start =
	    (dsize > SEARCH_REGION_SIZE) ? (dsize - SEARCH_REGION_SIZE) : 0;
	...
	while (pos < search_end) {
		...
		size_t bytes_to_read =
		    (search_end - pos >=
		     READ_BLOCK_SIZE) ? READ_BLOCK_SIZE : (search_end - pos);
		...
		if (bytes_read <= 0) {
			perror("read");
			dprintf
			    ("%s: Failed to read. Position=%llu, Bytes to read=%zu. Skipping.\n",
			     __func__, pos, bytes_to_read);
		...
		for (size_t offset = 0; offset + sizeof(be32) <= ull_bytes_read;
		     offset += sizeof(be32)) {
...
static int load_ddf_headers(int fd, struct ddf_super *super, char *devname)
{
	...
	if (!found_anchor) {
		...
		if (lseek64(fd, ddfpos, SEEK_SET) == -1L) {
			if (devname) {
				pr_err
				    ("Cannot seek to found DDF header position on %s: %s\n",
				     devname, strerror(errno));
			}
			return 1;
		}
